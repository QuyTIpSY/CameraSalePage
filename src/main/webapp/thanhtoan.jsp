		...
		<div id="wp-content">
			...
			<table>
				<%
					...
					<%}%>
					<tr>
							<td style="padding: 20px 0px;font-weight: bold;font-size: 20px;">Tổng tiền: </td>
			...
			<%if(ghbo!=null){ %>
				<div id="wp-content-thanhtoan">
				<p>Thông tin địa chỉ giao hàng</p><br>
				<div style="display: flex;justify-content: center;margin-top: 1px;">
					<form action="ktttController" method="post">		
						<input class="input" type="text" placeholder="Họ tên *" name="hoten" required="required"><br><br>
						<input class="input" type="tel" placeholder="Số điện thoại *" name="sdt" pattern="[0-9]{10}" required="required"><br><br>
						<input class="input" type="email" placeholder="Email *" name="email" required="required"><br><br>
						<input class="input" type="text" placeholder="Địa chỉ *" name="diachi" required="required"><br><br>
						<input style="border: 1px solid red;background: red;color: white;font-size: 20px;font-weight: bold;margin-left: 30px;" type="submit" value="Thanh toán">
					</form>
				</div>
			</div>
			<%} %>
			...
			<%if(ghbo==null){ %>
				<div>Chưa có thông tin hàng để thanh toán. Mời bạn <a href="trangchuController">MUA HÀNG</a></div>
			<%} %>
